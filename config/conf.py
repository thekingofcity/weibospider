...
instance_config_path = os.path.join(os.path.dirname(__file__), 'instance.yaml')

with open(config_path, encoding='utf-8') as f:
    cont = f.read()
cf = load(cont)

instance_specfic = False
if os.path.exists(instance_config_path):
    instance_specfic = True
    with open(instance_config_path, encoding='utf-8') as f:
        instance_cont = f.read()
    instance_cf = load(instance_cont)
...
def get_crawl_interal():
    interal = random.randint(cf.get('min_crawl_interal'),
                             cf.get('max_crawl_interal'))
    return interal
...
def get_broker_and_backend():
    ...
    if sentinel_args:
        broker_url = ";".join('sentinel://:{}@{}:{}/{}'.format(
            password, sentinel['host'], sentinel['port'], db)
                              for sentinel in sentinel_args)
        return broker_url
    else:
        ...
        backend_url = 'redis://:{}@{}:{}/{}'.format(password, host, port,
                                                    backend_db)
...
def get_images_path():
    img_dir = cf.get('images_path') if cf.get('images_path') else os.path.join(
        str(Path.home()), 'weibospider', 'images')
...
def get_adapter_ip():
    if instance_specfic:
        return instance_cf.get('adapter_ip')
    return None
